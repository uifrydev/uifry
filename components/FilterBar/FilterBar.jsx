...
const FilterBar = ({ filter, setFilter ,setCards,initialData,buttons=[],isFilter}) => {
  ...
  return (
    <div className="flex z-[1] lg:flex-col w-full items-start lg:pl-[2rem] pl-[23.4rem] pb-[2rem] bg-[#ffffff] sticky top-[8.41rem] pt-[2rem] lg:top-[20rem] pr-[4rem]">
      <div className="flex-1 flex gap-[1.6rem] flex-wrap lg:hidden">
        {buttons.map((item, index) => (
          <Button
            onClick={() => {
              setFilter((prev) => ({ ...prev, subCategory: item.title }));
              applyFilter({ ...filter, subCategory: item.title },setCards,initialData);
            }}
            key={index}
      ...
      <div className="hidden  w-full lg:flex py-[2rem]">
        <select
          ...
          onChange={(e) => {
            ...
            applyFilter({ ...filter, subCategory: e.target.value },setCards,initialData);
          }}
      ...
      {isFilter!==false && <div className="flex gap-[1rem] items-center pt-[.6rem]">
        ...
        {images.map((item, index) => (
          <Button
            onClick={() => {
              ...
              applyFilter({ ...filter, [item.title]: !filter[item.title] },setCards,initialData);
            }}
            key={index}
